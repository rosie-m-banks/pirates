...
export const ScoringConfig = {
  ...
  weights: {
    frequency: 1.0,  // How much to value word frequency (higher = prefer common words)
    length: 2.0,     // How much to value word length (higher = prefer longer words)
  },
  ...
  normalize: {
    ...
    length: false,     // Keep length as raw value (typically 3-15)
  },
};
...
export function scoreWord(word, frequency, config = ScoringConfig) {
  ...
  const normalizedFreq = config.normalize.frequency ? frequency / 8.0 : frequency;
 ...
 */
export function sortRecommendations(recommendedWords, frequencies, config = ScoringConfig) {
 ...
 */
export function getScoreStats(recommendedWords, frequencies, config = ScoringConfig) {
  const words = Object.keys(recommendedWords);
  const scores = words.map((word) => scoreWord(word, frequencies[word] || 0, config));
  ...
  return {
    ...
    frequencies: {
      ...
      avg: words.reduce((sum, w) => sum + (frequencies[w] || 0), 0) / words.length,
    },
    lengths: {
